...
async function preloadIndex(data: any) {
  ...
  for (const route in data) {
    ...
    for (let i = 0; i < urlParts.length; i++) {
      ...
      if (!crumbData) {
        logger.debug(
          `Excluded page ${searchUrl} - will not be included for search result breadcrumb for ${route}`,
        );
        continue;
      }
...
async function search(query: string): Promise<SearchResult[]> {
  const pageResults =
    pageIndex.search(query, 5, { enrich: true, suggest: true })[0]?.result ||
    [];
