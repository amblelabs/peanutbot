...
const SUPPORT_TYPES: [string, SupportType][] = Object.entries(
...
async function provideSupport(content: string): Promise<SupportSearchResult> {
  ...
  for (const [k, { keywords, question, answers }] of SUPPORT_TYPES) {
    ...
    const weight = countKeywords(content, keywords, (keyword) => {
      keywordBuilder.push(format(config.support.format.keywords, keyword));
    });
    ...
    const msgBuilder = [
      ...
      format(config.support.format.footer, {
        type: k,
        keywords: keywordBuilder,
        weight,
      }),
    ];
