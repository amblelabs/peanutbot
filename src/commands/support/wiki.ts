...
function makeReply(type?: string): string {
  if (type === "stargate" || type === "sg") {
    return config.support.short.wikiStargate;
  }

  return config.support.short.wikiAit;
}
...
function slash(builder: SlashCommandBuilder): SharedSlashCommand {
  return builder
    .setDescription("Sends a link to the wiki.")
    .addStringOption((option) =>
      option
        .setName("target")
        .setDescription("Which wiki?")
        .setChoices(
          { name: "Adventures in Time", value: "ait" },
          { name: "Stargate", value: "stargate" },
          { name: "Timeless Heroes", value: "th" },
        ),
    );
}
