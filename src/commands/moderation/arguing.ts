...
async function onMessage(ctx: Ctx, message: Message) {
  if (message.channelId !== config.arguing.channel) return;

  const content = message.content.toLowerCase();
  if (
    !message.mentions.has(ctx.client.user!) ||
    !content.includes("arguing") ||
    !content.includes("dev") ||
    !content.includes("chat")
  )
    return;

  const oneHourAgo = Date.now() - config.arguing.period;
  const messages = await message.channel.messages.fetch({
    limit: config.arguing.messages,
  });
  ...
  for (const memberId of members) {
    try {
      ...
      await member.timeout(config.arguing.period, "peanut: arguing");
    } catch (e) {}
  }
}
